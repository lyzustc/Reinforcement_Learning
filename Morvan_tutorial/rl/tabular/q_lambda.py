...
class q_lambda(tabular):
    def __init__(self, actions, alpha=0.01, epsilon=0.9, gamma=0.9, lambda_=0.9):
        super(q_lambda, self).__init__(actions, alpha, epsilon, gamma)
    ...
    def learn(self):
        ...
        self.table = self.table + self.alpha * (q_new - q_old) * self.eligibility_trace
        
        if(s_new != 'terminal' and a_star != a_new):
